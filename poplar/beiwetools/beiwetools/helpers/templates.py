...
class ProcessTemplate():
    '''
    This class is a template for raw data processing tasks, e.g. summary, 
    calibration, etc.  The purpose of this class is to get the do() method, 
    a function that processes lots of files.  The inputs for do() might be: 
        - A list of users, 
    ...
    - Step (1) may also include a function to configure logging output.
    - Steps (2a), (2b), (2c) could be captured in a single task, but are split 
      up for legibility.
    - Step (2a) may include functions that:
        - Create specific files or folders for the user's processed data,
        - Look up an alternate identifier for the user,
    ...
    - Functions in (2c) may write to files or folders created in steps (1) and/or (2a).
	- Functions in (3) may write to files or folders created in step (1).

    The do() function manages kwargs while processing tasks. Therefore:
        - Keywords should be used consistently everywhere.
        - If a function returns something, it should be in the form of a 
    ...
    def do(self, user_ids, process_kwargs, id_lookup = {}):
        ...
        for i in range(n):
            ...
            logger.info('%s: Setting up for user %s of %s...' % (self.name, i+1, n))           
            uid = user_ids[i]
