...
the logging package in Beiwe's Forest modules.
...
def f(x):
    logging.info('We are going to do some arithmetic.')
    y = 1/x
    return(y)
    
# To document an event with context:
def f(x):
    logging.info('We are going to do some arithmetic with x = %s.' % x)
    y = 1/x
    return(y)
...
def f(x):
    if x == 0:
        logging.warning('There may be a problem with this value of x.')
    y = 1/x
    return(y)
...
def f(x):
    try:
        y = 1/x
        logging.info('We did some arithmetic.') # Optional success message.
    except:
        y = None
        logging.warning('There may be a problem with x = %s.' % x)
    return(y)
...
logging.critical('This is a critical event!') # Something horrible happened.
...
# For basic logging written to CSV, make four changes:
import logging                        # 1. Import the logging package.
from base_functions import log_to_csv # 2. Import this function.
from forest_module import f
log_dir = 'path/to/output/directory'  # 3. Choose where to write log records.
log_to_csv(log_dir)                   # 4. Initialize log file and handler.
f(x)

# For comprehensive logging written to CSV, including traceback information:
import logging
from base_functions import traceback_format, log_to_csv
from forest_module import f
...
import logging                        
from base_functions import attributes_to_csv, log_to_csv
from forest_module import f
log_dir = 'path/to/output/directory'  
...
import logging                        
from base_functions import log_to_csv
...
import logging
from base_functions import log_to_csv   # 1. Import this function.
from some_other_forest_module import f, g, h
