...
"""
Compare output of summary API with output of Forest and print the result to stdout in CSV format
"""
...
parser.add_argument("api_file", help="path to summary API output file", type=str)
parser.add_argument("forest_file", help="path to Forest output file", type=str)
parser.add_argument(
    "participant_id", help="study participant ID", type=str, nargs="?", default=None
)
args = parser.parse_args()

if not args.participant_id:
    args.participant_id = os.path.splitext(os.path.basename(args.forest_file))[0]
...
with open(args.forest_file) as forest_file:
    for row in csv.DictReader(forest_file):
        d = f"{int(float(row['year']))}-{int(float(row['month'])):02}-{int(float(row['day'])):02}"
        forest_output[d] = row
...
with open(args.api_file) as api_file:
    ...
    for api_entry in api_output:
        if api_entry["participant_id"] != args.participant_id:
            continue
        d = api_entry["date"]
        forest_entry = forest_output[d]
        output_row = [d]
        for json_name, csv_name in variable_mapping.items():
            try:
                output_row.append(api_entry[json_name] - float(forest_entry[csv_name]))
            except KeyError:
                # ignore missing variables in Forest output
                output_row.append("")
            except ValueError:
                if api_entry[json_name] is None and forest_entry[csv_name] == "":
                    # ignore empty values
                    output_row.append("")
                    continue
                else:
