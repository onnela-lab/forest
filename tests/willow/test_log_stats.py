...
from forest.willow.log_stats import comm_logs_summaries, get_call_reciprocity, get_mean_responsiveness
...
def test_multiple_received_and_sent():
    ...
    result = get_mean_responsiveness(df, "direction", ["received"], ["sent"])
    ## Each sent message is 1000 ms after a received message, so the result should be 1 secondd
...
def test_sent_before_received_filtered_out():
    ...
    # sent timestamp is before received timestamp, should be filtered out, no pairs left
    result = get_mean_responsiveness(df, "direction", ["received"], ["sent"])
    assert pd.isna(result)
