...
def test_find_walking(signal_bout, fs, min_amp, step_freq, alpha, beta, min_t, delta):
    timestamp, _, x, y, z = signal_bout
    vm_bout = preprocess_bout(timestamp, x, y, z)[1]
    cadence_bout = find_walking(vm_bout, fs, min_amp, step_freq, alpha, beta, min_t, delta)
    expected_output = np.array([1.65, 1.6, 1.55, 1.6, 1.55, 1.85, 1.8, 1.75, 1.75, 1.7])
