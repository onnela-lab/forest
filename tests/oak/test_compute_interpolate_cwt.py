...
from forest.oak.analysis import compute_interpolate_cwt
...
def test_compute_interpolate_cwt(signal_bout, fs, wavelet):
    ...
    expected_output_amp = 0.000482
    expected_output_freqs = np.arange(0.5, 4.5, 0.05)
    assert math.isclose(np.max(coefs_interp), expected_output_amp, abs_tol=1e-4)
