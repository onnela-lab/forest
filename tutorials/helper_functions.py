...
# keys for the keyring dictionary
USERNAME, PASSWORD, ACCESS_KEY, SECRET_KEY = "USERNAME", "PASSWORD", "ACCESS_KEY", "SECRET_KEY"
    ...
    def inner(directory: Path, prefix: str = "", level2: int = -1):
        ...
        for pointer, path in zip(pointers, contents):
            ...
            if path.is_dir():
                ...
                yield from inner(path, prefix + extension, level2 - 1)
...
def concatenate_summaries(dir_path: Path | str, output_filename: str):
...
def concatenate_folder(dir_path: Path, output_filename: str):
    ...
    for file in os.listdir(dir_path):  # loop through files in dir_path
...
def download_data(
    ...
    This function downloads all data for selected users, time frame, and data streams, and writes
    them to an output folder, with one subfolder for each user, and subfolders inside the user's
    folder for each data stream. If a server failure happens, the function re-attempts the download.
    ...
    Args:
        ...
        time_start(str): The initial date to download data
            (Formatted in YYYY-MM-DD). Default is 2008-01-01, before any Beiwe data could exist.
    ...
    if not any([keyring[USERNAME], keyring[PASSWORD], keyring[ACCESS_KEY], keyring[SECRET_KEY]]):
        print("Error: Did you set up the keyring_studies.py file?")
        return
    
    # print our ~deprecation warning (users should use directly Mano)
    print(
        "Please note that this tutorials.download_data function has been superceded by improvements"
        " to the Mano library. Visit https://github.com/onnela-lab/mano for more information.\n"
    )
    ...
    if not users:
        ...
        for _ in range(5):
            try:
                users = list(mano.users(keyring, study_id))
                break
                ...
                print(e)
                sys.exit(1)
            except Exception as e:  # other exceptions are usually network dropouts, will retry
                print(f"encountered error obtaining list of participant users: {e}")
    
    if not users:
        print("No users found for this study, or network failure.")
        sys.exit(1)
    ...
    for user in users:
        ...
        num_tries = 0
        while num_tries < 5:
            print(f"Downloading data for {user}")
            try:
                zf = msync.download(
                    keyring, study_id, user, data_streams, time_start=time_start, time_end=time_end
                )
            except requests.exceptions.ChunkedEncodingError:
                print(f"Network failed in download of {user}, try {num_tries}")
                num_tries += 1
            except KeyboardInterrupt:
                print("Someone closed the program")
                sys.exit(1)
            ...
            print(f"Too many failures; skipping participant user {user}")
        
        if zf is None:
            print(f"No data for {user}; nothing written")
        else:
