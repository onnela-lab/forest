...
path2 = "C:/Users/glius/Google Drive/HOPE/detailed_clinical_events/HOPETrial_DATA_2019-12-06_1619.csv"
path3 = "C:/Users/glius/Downloads/test"
```
...
heatmap = function(id,path1,path2=NULL,path3=NULL,option){
  ...
  if(sum(!is.na(temp$value))>0){
    ...
    if(is.null(path2)==F){
      ...
      new_set = subset(new_set,x_day<=max(daily_summary$key))
    ...
    if(option=="show"){
      ...
      if(is.null(path3)){
        ggsave(paste0(id,"_heatmap.pdf"), plot = graph, device = "pdf", path = NULL,scale = 1, width = 20, height = 5, units = "in",dpi = 300, limitsize = TRUE)
        print(paste0(id," is Done."))
      }
      else{
        ggsave(paste0(path3,"/",id,"_heatmap.pdf"), plot = graph, device = "pdf", path = NULL,scale = 1, width = 20, height = 5, units = "in",dpi = 300, limitsize = TRUE)
        print(paste0(id," is Done."))
      }
    }
    else{
    ...
    for(i in 1:k){
      ...
      if(option2=="show"){
        ...
        if(is.null(path3)){
          ...
          print(paste0(id,": Month ",i, " is Done."))
        }
        else{
