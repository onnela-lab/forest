...
# poplar
This directory contains the `beiwetools` package.  This package provides tools for managing and documenting Beiwe study settings and raw data sets.  There are currently three sub-packages:
...
# acacia
This directory contains the `logrep` package, which provides tools for processing raw Beiwe app log files from Android devices.

# ash
This directory contains the `accrep` package, which provides tools for processing raw Beiwe accelerometer data.

# fir
This folder contains the `fitrep` package.  Use these tools to reformat Fitbit data delivered by the Fitabase platform.

# ginkgo
This directory contains the `gpsrep` package, which includes tools for some basic GPS data processing tasks such as time zone extraction.

# pine
This directory contains the `powrep` package, which provides tools for processing raw Beiwe power state files.

# sycamore
This directory contains the `survrep` package, which provides tools for processing raw Beiwe tracking survey data.
