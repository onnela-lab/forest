...
Several open-source tools now analyze data of this kind. Cortex is a data processing pipeline optimized for the mindLAMP apps, deriving clinical features, quality metrics and visualizations [@burns2024cortex; @vaidyam2022lamp], while RAPIDS is a reproducible workflow that standardizes preprocessing and feature extraction across Android and iOS sensing applications as well as Fitbit and Empatica devices [@vega2021rapids]. Both prioritize breadth of supported data sources and reproducibility of the pipeline that turns raw streams into features. Forest is organized around a different problem: estimating behavioral quantities using methods with published statistical properties, treating the irregular sampling and substantial missingness of smartphone data as something to be modeled rather than summarized [@barnett2020inferring; @liu2021bidirectional]. Implementing such methods inside a feature extraction pipeline would have required either weakening them or restructuring a project built on different assumptions, so the tools are complementary rather than competing.
