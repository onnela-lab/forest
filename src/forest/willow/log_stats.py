...
def get_mean_responsiveness(
    ...
    Args:
        ...
        col_with_sent_received: The column indicating whether a message is sent or received
        
        received_values_list: values of col_with_sent_received indicating that a message/call was
            received
        
        sent_values_list: values of col_with_sent_received indicating that amessage/call was sent
    
    Returns:
        float or a pd.NA: The average time between having a call sent or recieved
    """
    cols = ["hashed phone number", "timestamp"]
    received = df.loc[df[col_with_sent_received].isin(received_values_list), cols]
    sent = df.loc[df[col_with_sent_received].isin(sent_values_list), cols]
    
    # Joining the dataframe to itself will filter to only messages that are received
    joined = pd.merge(
        left=sent,
        right=received,
        how="inner",
    ...
    # We only want to take the first row by received timestamp because that will be the earliest
    # possible sent time
    ...
    mean_responsiveness_ms = (joined["timestamp_sent"] - joined["timestamp_received"]).mean()
    return mean_responsiveness_ms / 1_000 / 60  # minutes


def text_analysis(
    df_text: pd.DataFrame, stamp: int, step_size: int, frequency: Frequency
) -> dict[str, int | float]:
    """ Calculate the summary statistics for the text data in the given time interval.
    ...
    temp_text = df_text[
        (df_text["timestamp"] / 1000 >= stamp) & (df_text["timestamp"] / 1000 < stamp + step_size)
    ]
    ...
    if frequency == Frequency.DAILY:
        ...
        received_no_response = [tel for tel in receive_from_number if tel not in send_to_number]
        sent_no_response = [tel for tel in send_to_number if tel not in receive_from_number]
...
def text_and_call_analysis(
    ...
    """
    Calculate the summary statistics for anything requiring both call and text data in the given
    time interval.
    ...
    Returns:
        dict of summary statistics containing:
            num_uniq_individuals_call_or_text: int
                number of people making incoming calls or texts to the Beiwe user or who the Beiwe
                user made outgoing calls or texts to
    """
    # filter the data based on the timestamp
    ...
    num_uniq_individuals_call_or_text = len(
        np.unique(np.hstack([calls_in, texts_in, texts_out, calls_out, calls_mis]))
    )
    return {"num_uniq_individuals_call_or_text": num_uniq_individuals_call_or_text}


def get_call_reciprocity(calls_dict: dict) -> float | NAType:
    """
    Get call reciprocity for an individual.
        This is defined as 1 - (|incoming - outgoing|) / (incoming + outgoing).
    
    A reciprocity of 1 indicates perfect reciprocity--all incoming calls are balanced by an outgoing
    call. A reciprocity of 0 indicates that an individual had only incoming calls OR that they had
    only outgoing calls.
    
    Args:
       calls_dict: a dict with two keys: "incoming" and "outgoing".
           "incoming" includes a series with incoming phone numbers, "outgoing" has outgoing numbers
    ...
    for k in ["incoming", "outgoing"]:
        value_counts[k] = (
            ...
            .value_counts()
            .reset_index()
            .rename({"index": "hashed_phone_number", "count": "num_" + k}, axis=1)
        )
...
def call_analysis(df_call: pd.DataFrame, stamp: int, step_size: int) -> dict:
    """ Calculate the summary statistics for the call data in the given time interval.
    ...
    """
    # filter the data based on the timestamp
    temp_call = df_call[(df_call["timestamp"] / 1000 >= stamp) &
                        (df_call["timestamp"] / 1000 < stamp + step_size)]
...
def comm_logs_summaries(
    ...
    """
    all_summary_stats = []
    start_year, start_month, start_day, start_hour, _, _ = stamp2datetime(stamp_start, tz_str)
    end_year, end_month, end_day, end_hour, _, _ = stamp2datetime(stamp_end, tz_str)
    
    # determine the starting and ending timestamp again based on the frequency
    if frequency == Frequency.HOURLY_AND_DAILY:
        logger.error("Error: frequency cannot be HOURLY_AND_DAILY for this function")
    ...
    # determine the step size based on the frequency, step_size is in seconds
    ...
    for stamp in np.arange(table_start, table_end + 1, step=step_size):
        ...
        if df_text.shape[0] > 0 or df_call.shape[0] > 0:
            text_and_call_stats = text_and_call_analysis(df_call, df_text, int(stamp), step_size)
    ...
    for line_dict in all_summary_stats:
        current_line_dict = [line_dict.get(col, pd.NA) for col in columns_to_output]
...
def log_stats_main(
    ...
    """ Main function for calculating the summary statistics for the communication logs.
    ...
    if frequency not in [
        ...
        logger.error(
            "Error: frequency must be one of the following: HOURLY_AND_DAILY, DAILY, HOURLY"
        )
    ...
    # process the data for each participant in each frequency into a folder of the corresponding
    # frequency.
...
def log_stats_inner(
    ...
    # num_uniq_individuals_call_or_text is the cardinality of the union of several sets. It should
    # should always be at least as large as the cardinality of any one of the sets, and it should
    # never be larger than the sum of the cardinalities of all of the sets. (it may be equal if all
    # the sets are disjoint)
    num_uniq_column = "num_uniq_individuals_call_or_text"  # legibility hax.
    sum_all_set_cols = pd.Series([0] * stats_pdframe.shape[0])
    for column in ["num_s_tel", "num_r_tel", "num_in_caller", "num_out_caller", "num_mis_caller"]:
        ...
        if (
            ...
            logger.error(NUM_UNIQ_INDIVIDUALS_CALL_OR_TEXT_ERR_1_MSG, column)
    
    if (pd.to_numeric(stats_pdframe[num_uniq_column]).fillna(0) > sum_all_set_cols).any():
        logger.error(NUM_UNIQ_INDIVIDUALS_CALL_OR_TEXT_ERR_2_MSG)
    
    # merge year, month, day columns to date, and drop them, force left
    stats_pdframe["date"] = pd.to_datetime(stats_pdframe[["year", "month", "day"]])
...
#TODO these should probably go in some constants file

NUM_UNIQ_INDIVIDUALS_CALL_OR_TEXT_ERR_2_MSG = """
Error:  num_uniq_individuals_call_or_text was found to be larger than the sum of individual
cardinalities for at least one time interval.  This error comes from an issue with the code, not an
issue with the input data.
""".replace("\n", " ").strip()

NUM_UNIQ_INDIVIDUALS_CALL_OR_TEXT_ERR_1_MSG = """
Error: num_uniq_individuals_call_or_text was found to be less than %s for at least
one time interval. This error comes from an issue with the code, not an issue with
the input data.
""".replace("\n", " ").strip()
...
TEXT_COLUMNS_DAILY_ONLY = [
    "mean_responsiveness_text", "text_reciprocity_incoming", "text_reciprocity_outgoing"
]
