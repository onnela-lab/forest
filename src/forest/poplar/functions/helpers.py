...
def clean_dataframe(
    ...
    Args:
        ...
        sort (bool):  Sort by timestamp.  If True, df must have a 'timestamp' column.
    ...
    if sort:
        try:
            ...
            logger.exception("Unable to sort by timestamp. No column named timestamp")
        except TypeError:
            logger.exception("Unable to sort by timestamp. Multiple types in column")
...
def get_windows(
    ...
    """
    Generate evenly spaced windows over a time period.  For each window, figure out which rows of a
    data frame were observed during the window. Usually df contains an hour of raw sensor data, and
    start/end are the beginning/ending timestamps for that hour.
    ...
    Returns:
        windows (OrderedDict):  Keys are timestamps corresponding to the
            beginning of each window.  Values are lists of row indices in df that fall within the
            corresponding window.
    """
    if (end - start) % window_length_ms != 0:
        logger.warning(
            "The window length doesn't evenly divide the interval. Returning empty OrderedDict"
        )
    else:
        try:
            windows: OrderedDict = OrderedDict.fromkeys(np.arange(start, end, window_length_ms), [])
            for i in range(len(df)):
                key = df.timestamp[i] - (df.timestamp[i] % window_length_ms)
                windows[key].append(i)
            return windows
        except KeyError:
            logger.warning("Unable to identify windows, Returning empty OrderedDict")
...
def directory_size(dirpath: str, ndigits=1) -> tuple:
    ...
    Args:
        dirpath (str):  Path to a directory. ndigits (int): Precision.

    Returns:
        size (float): Size in megabytes. file_count (int): Number of files in the directory.
    """
    size = 0
    file_count = 0
...
def sort_by(list_to_sort: list, list_to_sort_by: list) -> list:
    """
    Sort a list according to items in a second list.  For example, sort person identifiers according
    to study entry date.
...
def sample_std(a: pd.DataFrame | np.ndarray | pd.Series | list) -> np.floating :
...
def sample_var(a: pd.DataFrame | np.ndarray | pd.Series | list) -> np.floating:
