...
def compute_survey_stats(
    ...
    Args:
        ...
        config_path:
            File path to study configuration file. Study configuration files can be downloaded by
            clicking "Edit this Study" on the Beiwe website, then clicking "Export study settings
        ...
        interventions_filepath:
            filepath where interventions json file is. The interventions json file can be downloaded
            by clicking "Edit this Study" on the Beiwe website, then clicking "Download
            Interventions" next to "Intervention Data".
        augment_with_answers:
            Whether to use the survey_answers stream to fill in missing surveys from survey_timings
        submits_timeframe:
            The timeframe to summarize survey submissions over of class forest.constants.Frequency .
            An overall summary for each user is always generated ("submits_summary_overall.csv"),
            and submissions can also be generated across days ("submits_summary_daily.csv"), hours
            ("submits_summary_hourly.csv") or both.
        submits_by_survey_id:
            Whether to summarize survey submits with separate lines for different surveys in
            submits_summary.csv. By default, this is True, so a different line for each survey will
            be generated.
        history_path: Path to survey history file.
            If this is included, the survey history file is used to find instances of commas or
            semicolons in answer choices to determine the correct choice for Android radio
            questions. In addition, this is used to generate timings for audio surveys. The survey
            history json file can be downloaded by clicking "Edit this Study" on the Beiwe website,
            then clicking clicking "Download Surveys" next to "Survey History".
    ...
    if submits_timeframe not in (Frequency.HOURLY_AND_DAILY, Frequency.HOURLY, Frequency.DAILY):
        logger.error("Error: Invalid submits timeframe")
        return False
    
    # setup folders
    summaries_folder = pathjoin(output_folder, "summaries")
    surveys_folder = pathjoin(output_folder, "by_survey")
    makedirs(output_folder, exist_ok=True)
    makedirs(summaries_folder, exist_ok=True)
    makedirs(surveys_folder, exist_ok=True)
    ...
    if config_path is None:
        ...
        to_csv(summaries_folder, "answers_data.csv", ca_detail)
        to_csv(summaries_folder, "answers_summary.csv", ca_summary)
        ...
        if start_date is not None and end_date is not None:
            ...
            ss_summary = summarize_submits(ss_detail, None, submits_by_survey_id)
            
            if ss_summary.shape[0] > 0:
                to_csv(summaries_folder, "submits_and_deliveries.csv", ss_detail)
                to_csv(summaries_folder, "submits_summary.csv", ss_summary)
                
                if submits_timeframe in (Freq.HOURLY, Freq.HOURLY_AND_DAILY):
                    ss_summary_h = summarize_submits(ss_detail, Freq.HOURLY, submits_by_survey_id)
                    to_csv(summaries_folder, "submits_summary_hourly.csv", ss_summary_h)
                
                if submits_timeframe in (Freq.DAILY, Freq.HOURLY_AND_DAILY):
                    ss_summary_d = summarize_submits(ss_detail, Freq.DAILY, submits_by_survey_id)
                    to_csv(summaries_folder, "submits_summary_daily.csv", ss_summary_d)
    ...
    for survey_id in surveys_dict:
        to_csv(surveys_folder, f"{survey_id}.csv", surveys_dict[survey_id])
    
    # Write out summaries
    to_csv(summaries_folder, "agg_survey_data.csv", agg_data)
    
    # Add alternative survey submits table
    submits_tbl = survey_submits_no_config(agg_data)
    to_csv(summaries_folder, "submits_only.csv", submits_tbl)
...
def to_csv(folder: str, filename: str, df: DataFrame) -> None:
    # very common patterm in this file otherwise creates a lot of boilerplate
    df.to_csv(pathjoin(folder, filename), index=False)
