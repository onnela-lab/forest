...
def generate_survey_times(
    ...
    intervention_dict: dict | None = None
) -> list:
    """Get delivery times for a survey
    
    Takes a start time and end time and generates a schedule of all sent surveys in time frame for
    the given survey type
    ...
    Returns:
        A list of all survey times that occur between the time_start and time_end per the given
        survey timings schedule
    """
    ...
    if survey_type == "weekly":
        ...
        weeks = math.ceil(weeks / 7.0)  # Get ceiling number of weeks
        
        t_lag = list(np.roll(np.array(timings, dtype="object"), -1))  # Roll dates
        
        # for each week, generate the survey times and append to a list
        start_dates = [t_start + datetime.timedelta(days=7 * i) for i in range(weeks)]
        
        for start in start_dates:
            for i, time in enumerate(t_lag):
                if len(time) > 0:
                    surveys.extend(convert_time_to_date(start, day=i, time_list=time))
    ...
    if survey_type == "relative":
        ...
        for time in timings:
            ...
            if intervention_dict[time[0]] is not None:
                ...
                current_time = (
                    datetime.datetime.fromisoformat(intervention_dict[time[0]]) +
                    pd.Timedelta(time[1], unit="days") + pd.Timedelta(time[2], unit="seconds")
                )  # seconds after time
...
def get_question_ids(survey_dict: dict[str, dict], audio_survey_id_dict: dict[str, str]) -> list:
    ...
    Args:
        ...
        audio_survey_id_dict: Output from get_audio_survey_id_dict.
            Dict with survey prompts as keys and survey IDs as values
    
    Returns:
        List of all question IDs for an individual. For audio surveys, it will create a "question
        ID" that is identical to the survey ID
    """
...
def gen_survey_schedule(
    ...
    This generates a DataFrame with the times surveys were delivered for each user. In addition,
    another delivery time is added a week after the last delivery time to catch the survey submitted
    ...
    Args:
        ...
        all_interventions_dict(dict):
            Dictionary containing a key for every beiwe id. Each value in the dict is a dict, with a
            key for each intervention and a timestamp for each intervention time
        history_path: Filepath to the survey history file. If this is not included, audio survey
            timings cannot be estimated.
    
    Returns:
        DataFrame with a line for every survey deployed to every user in the study for the given
        time range
    """
    ...
    times_sur = []
    for user in users:
        for i, survey in enumerate(surveys):  # For each survey create a list of survey times
            
            s_times: list = []
            if survey["timings"]:
                s_times = s_times + generate_survey_times(
                    time_start, time_end, timings=survey["timings"], survey_type="weekly"
                )
            if survey["absolute_timings"]:
                s_times = s_times + generate_survey_times(
                    time_start,
                    time_end,
                    timings=survey["absolute_timings"],
            ...
            # add a delivery time a week in the future to capture the last delivery
            week_after_last = tbl.delivery_time.max() + pd.Timedelta(7, "D")
            
            tbl = pd.concat([tbl, pd.DataFrame({"delivery_time": [week_after_last]})], axis=0)
...
def survey_submits(
    ...
    Returns:
        A DataFrame with deployment time and information about each user's response to a survey for
        all surveys deployed in the given timeframe.
    """
    ...
    # First, figure out if they opened the survey (if there are any lines related to the survey).
    # They get an opened_line if their current surv_inst_flg doesn't match the previous or if the
    # current survey id doesn't match the previous.
    
    # In some surveys, there will be multiple config_ids for the same line because question IDs are
    # duplicated across surveys. So, all of these config_ids get set with 1 because they all
    # happened at the same time.
    for user in users:
        opening_times = agg.loc[
            (agg.beiwe_id == user) & (
                (agg.surv_inst_flg != agg["surv_inst_flg"].shift(1, fill_value=-1)) |
                (agg["survey id"] != agg["survey id"].shift(1, fill_value="fill"))
            ),
            "Local time",
        ].unique()
    ...
    # Note: np.nan, None, and [] cannot be passed to isin() on a StringArray (pandas nullable string
    # dtype) without raising errors in newer pandas versions. Instead, handle nulls with isna() and
    # keep only string values in isin().
    only_real_answers = agg.loc[
        ~(agg.answer.isna() | agg.answer.isin(["", "[]", "NO_ANSWER_SELECTED"])),
    ...
    # When we do the join, we don't want to create any additional rows in the agg dataframe by
    # having duplicate rows. We also don't want to have the duplicate column.
    ...
    agg = pd.merge(
        agg,
        only_real_answers,
        # We have to drop 'question id' so that rows with no answer will still get a
    ...
    # Merge survey submit lines onto the schedule data and identify submitted lines. This creates a
    # very long dataframe with all survey submissions/openings on lines with each delivery
    submit_lines = pd.merge(
        sched[["delivery_time", "next_delivery_time", "id", "beiwe_id"]].drop_duplicates(),
        agg[[
            "Local time", "config_id", "survey id", "beiwe_id", "submit_line", "opened_line",
            "start_time", "num_questions_answered"
        ]].loc[(agg.submit_line == 1) | (agg.opened_line == 1)].drop_duplicates(),
        how="left",
        left_on=["id", "beiwe_id"],
    ...
    submit_lines3 = pd.merge(
        submit_lines2,
        submit_lines[merge_cols + ["Local time", "start_time", "num_questions_answered"]],
        how="left",
        left_on=merge_cols,
        right_on=merge_cols,
    )
    ...
    submit_lines3["time_to_open"] = (submit_lines3["start_time"] -
                                     submit_lines3["delivery_time"]).dt.total_seconds()
    submit_lines3["survey_duration"] = (submit_lines3["submit_time"] -
                                        submit_lines3["start_time"]).dt.total_seconds()
    ...
    submit_lines3["survey_duration"] = np.where(
        submit_lines3["survey_duration"] == 0, np.nan, submit_lines3["survey_duration"]
    )
    ...
    submit_lines3["survey_duration"] = np.where(
        submit_lines3["submit_flg"] == 0, np.nan, submit_lines3["survey_duration"]
    )
    ...
    submit_lines3["time_to_submit"] = np.where(
        submit_lines3["submit_flg"] == 1, submit_lines3["time_to_submit"], np.nan
    )
    submit_lines3["time_to_open"] = np.where(
        submit_lines3["opened_flg"] == 1, submit_lines3["time_to_open"], np.nan
    )
    return submit_lines3.sort_values(["survey id", "beiwe_id"]).drop_duplicates()


def summarize_submits(
    submits_df: pd.DataFrame,
    timeunit: Frequency | None = None,
    summarize_over_survey: bool = True,
) -> pd.DataFrame:
    """Summarize a survey submits df
    
    This generates the number of surveys opened and submitted, as well as the average time to open
    and complete a survey over a period of time
    
    Args:
        submits_df: output from survey_submits
        
        timeunit: One of None, Frequency.HOURLY or Frequency.DAILY.
            The unit of time on which to summarize the submits. If this is None, submits are
            summarized over the Beiwe ID and survey ID only. If this is "Day" or "Hour", submits are
            summarized over either the day or hour in which they were delivered.
        
        summarize_over_survey: Whether to summarize over survey.
            If this is True, the output will include a separate row for each survey ID within each
            time interval. If this is False, the output will take sums and means over all surveys.
            (i.e. num_surveys_submitted will include submissions from more than one survey)
    ...
    # If there is no submit data (e.g. no survey schedule was found upstream), there is nothing to
    # summarize. Return an empty frame with the columns the non-empty path would produce, so callers
    ...
    if submits.shape[0] == 0 or "delivery_time" not in submits.columns:
        ...
        if timeunit in (Frequency.HOURLY_AND_DAILY, Frequency.DAILY, Frequency.HOURLY):
        ...
        return pd.DataFrame(
            columns=empty_summary_cols + [
                "num_surveys", "num_complete_surveys", "num_opened_surveys", "avg_time_to_submit",
    ...
    if timeunit == Frequency.HOURLY_AND_DAILY:
        logger.warning(
            "Error: summarize_submits cannot calculate both daily and hourly summaries at one time."
    ...
    if np.sum(submits.submit_flg == 1) > 0:
        # this will work (and only makes sense) if there is at least one row with a survey submit
        avg_time_to_submit = (
            submits.loc[submits.submit_flg == 1].groupby(summary_cols)
            ["time_to_submit"].apply(lambda x: np.mean(x))
        )
        
        avg_time_to_open = (
            submits.loc[submits.opened_flg == 1].groupby(summary_cols)
            ["time_to_open"].apply(lambda x: np.mean(x))
        )
        avg_duration = (
            submits.loc[submits.opened_flg == 1].groupby(summary_cols)
            ["survey_duration"].apply(lambda x: np.mean(x))
        )
    else:
...
def survey_submits_no_config(input_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    agg = (
        agg.groupby(["survey id", "beiwe_id", "surv_inst_flg"])["Local time"]
        .apply(summarize_submission).reset_index()
    )
    agg = agg.pivot(
        index=["survey id", "beiwe_id", "surv_inst_flg"], columns="level_3", values="Local time"
    ).reset_index()
    agg["time_to_complete"] = agg["max_time"] - agg["min_time"]
    agg["time_to_complete"] = [time.seconds for time in agg["time_to_complete"]]
...
def get_all_interventions_dict(filepath: str | None) -> dict:
    ...
    Args:
        filepath: the path to a json file containing patient interventions information.
        (downloaded from the beiwe website or via mano)
    
    Returns:
        a dict with one key for each beiwe_id in the study. The value for each key is a dict with a
        key for each intervention in the study, and a value which is the timestamp
    """
    if filepath is None:
        return {}
