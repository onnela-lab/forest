...
def safe_read_csv(filepath: str) -> pd.DataFrame:
    ...
    Returns:
        A pandas DataFrame with information in the csv file (if the file is formatted correctly), or
        a blank pandas DataFrame.
    """
    try:
...
def standardize_question_type(question: str, lkp: dict | None = None) -> str:
...
def read_and_aggregate(
    ...
    Reads in all survey_timings data for a particular user and data stream and stacks the datasets
    
    Args:
        study_dir (str):
            path to downloaded data. This is a folder that includes the user data in a subfolder
            with the beiwe_id as the subfolder name
        user (str):
    ...
    Returns:
        Dataframe with stacked data, a field for the beiwe ID, a field for the day of week.
    """
    if time_end is None:
    ...
    if isdirectory(st_path):
        ...
        survey_data: pd.DataFrame = pd.concat(survey_data_list, axis=0, ignore_index=False)
...
def aggregate_surveys(
    ...
    Reads all survey_timings data from a downloaded study folder and stacks data together.
    Standardizes question types between iOS and Android devices.
    ...
    Returns:
        A DataFrame that has a question index field to understand if there are multiple lines for
        one question.
    """
    # READ AND AGGREGATE DATA
    # get a list of users (ignoring hidden files and registry file downloaded when using mano)
    users = users or get_ids(study_dir)
    ...
    for user in users:
        all_data_list.append(
            read_and_aggregate(study_dir, user, "survey_timings", time_start, time_end, tz_str)
        )
    
    # Collapse all users into one file and drop duplicates
    all_data: pd.DataFrame = pd.concat(all_data_list, axis=0, ignore_index=False)
    ...
    # Replace the question ID in these rows with a valid question ID for the survey so that survey
    # scheduling metrics can incorporate surveys that were opened.
    
    # If the only other rows in all_data were rows describing that the survey was first rendered,
    # put "" for the question ID.
    all_data["question id"] = all_data.apply(
        lambda row: (
            all_data.loc[
                (all_data["survey id"] == row["survey id"]) &
                (all_data["question id"] != all_data["event"]), "question id"].tolist() + [""]
        )[0] if row["question id"] == row["event"] else row["question id"],
        axis=1
    )
    
    # Fix question types
    all_data["question type"] = all_data.apply(
        lambda row: standardize_question_type(row["question type"], QUESTION_TYPES_LOOKUP), axis=1
    )
    ...
    all_data["question index"] = all_data.apply(
        lambda row: 1 if (row["question id"] != row["question id lag"]) else 0,
        axis=1,
    )
    all_data["question index"] = \
        all_data.groupby(["survey id", "beiwe_id"])["question index"].cumsum()
    ...
    all_data["surv_inst_flg"] = \
        all_data.groupby(["survey id", "beiwe_id"])["surv_inst_flg"].cumsum()
...
def parse_surveys(config_path: str, answers_l: bool = False) -> pd.DataFrame:
    ...
    output = []  # Create an array for surveys and one for timings
...
def convert_timezone_df(
    df_merged: pd.DataFrame, tz_str: str = "UTC", utc_col: str = "UTC time"
) -> pd.DataFrame:
    ...
    df_merged["Local time"] = \
        df_merged[utc_col].dt.tz_localize("UTC").dt.tz_convert(tz_str).dt.tz_localize(None)
...
def aggregate_surveys_config(
    ...
    Merges stacked survey data with processed configuration file data and removes lines that are not
    questions or submission lines. Uses survey_answers to fill in missing survey_timings data when
    needed.
    ...
    Args:
        ...
        augment_with_answers:
            Whether to use the survey_answers stream to fill in missing surveys from survey_timings
        time_start:
            The first date of the survey data, in YYYY-MM-DD format
        time_end:
            The last date of the survey data, in YYYY-MM-DD format
        history_path:
            Path to survey history file. If this is included, the survey history file is used to
            find instances of commas or semicolons in answer choices to determine the correct choice
            for Android radio questions
        include_audio_surveys:
            Whether to include submissions of audio surveys in addition to text surveys
    ...
    agg_data = aggregate_surveys(study_dir, users, time_start, time_end, study_tz)
    if agg_data.shape[0] == 0:
        return agg_data
    
    # Merge data together and add configuration survey ID to all lines. Pandas gives an error if
    # question IDs are different data types, so, we will convert everything to string.
    # But, if we just do raw conversion to string, we run into problems where it tries to merge on
    # 'nan'. So, we will convert all of the 'nan' to None before merging.
    ...
    df_merged["config_id"] = df_merged.apply(
        lambda row: row["config_id_update"]
        if row["event"] in ["User hit submit", "submitted"] else row["config_id"],
        axis=1,
    )
    ...
    df_merged["submit_line"] = df_merged.apply(
        lambda row: 1 if row["event"] in ["User hit submit", "submitted"] else 0,
        axis=1,
    )
    
    # Remove notification and expiration lines
    df_merged = df_merged.loc[
        (~df_merged["question id"].isnull()) | (~df_merged["config_id"].isnull())
    ]
    ...
    if augment_with_answers:
        ...
        df_merged = append_from_answers(
            df_merged, study_dir, users, study_tz, time_start, time_end, config_path, history_path
        )
    if include_audio_surveys:
        audio_surveys = read_aggregate_audio_recordings_stream(
            study_dir, users, study_tz, config_path, time_start, time_end, history_path
        )
        df_merged = pd.concat([df_merged, audio_surveys], axis=0, ignore_index=False)
    # mark NO_ANSWER_SELECTED as na because researchers will treat these the same, and researchers
    # get confused if they see this as a possible answer that is distinct from na
    df_merged.loc[df_merged["answer"] == "NO_ANSWER_SELECTED", "answer"] = np.nan
...
def aggregate_surveys_no_config(
    ...
    Args:
        ...
        augment_with_answers(bool):
            Whether to use the survey_answers stream to fill in missing surveys from survey_timings
        include_audio_surveys:
            Whether to include submissions of audio surveys in addition to text surveys. Default is
            True
    ...
    # Remove lines where the event is 'notified' or 'expired'. These lines will show up on file
    # collected from iOS devices, and they happen when a user gets notified of a survey and the
    # survey expires because another survey gets sent.
    ...
    if augment_with_answers:
        ...
        agg_data = append_from_answers(agg_data, study_dir, users, study_tz, time_start, time_end)
    
    if include_audio_surveys:
        audio_surveys = read_aggregate_audio_recordings_stream(
            study_dir, users, study_tz, None, time_start, time_end, None
        )
        agg_data = pd.concat([agg_data, audio_surveys], axis=0, ignore_index=False)
...
def append_from_answers(
    ...
    Sometimes, a survey is missing from survey_timings. If survey_answers contains a survey that is
    missing from survey_timings, this function adds the less complete information from
    survey_answers to fill in the gaps.
    ...
    Args:
        ...
        download_folder:
            ...
            with the beiwe_id as the subfolder name
        tz_str:
            Timezone to use for "Local time" column values. This defaults to "UTC"
        users:
    ...
    Returns:
        Data frame with all survey_timings data, including data from survey_answers where
        survey_timings data is missing.
    """
    time_end = time_end or get_month_from_today()
    answers_data = read_aggregate_answers_stream(
        download_folder, users, tz_str, config_path, time_start, time_end, history_path
    )
    if answers_data.shape[0] == 0:
        return agg_data
    
    users = users or get_ids(download_folder)
    missing_submission_data = []  # list of surveys to add on to end
    
    for user in users:
        for survey_id in agg_data["survey id"].unique():
            missing_data = find_missing_data(user, survey_id, agg_data, answers_data)
            if missing_data.shape[0] == 0:
                continue
            missing_data["question index"] += np.max(agg_data["question index"]) + 1
            missing_submission_data.append(missing_data)
    
    return pd.concat([agg_data] + missing_submission_data).sort_values("UTC time")
...
def find_missing_data(user: str, survey_id: str, agg_data: pd.DataFrame,
    ...
    Args:
        user: String with the Beiwe ID to look for missing data survey_id: String with the survey ID
        to look for missing data in
        
        agg_data: Dataframe with aggregated timings data
            (output from aggregate_surveys_config)
        answers_data: Dataframe with aggregated answers data
            (output from read_aggregate_answers_stream)
    ...
    for time in known_answers_submits:
        # If there were no timings submits recorded, every answers submit will be missing
        if len(known_timings_submits) == 0:
            missing_times.append(time)
            continue
        
        hours_from_nearest = np.min(
            np.abs((pd.to_datetime(known_timings_submits) - pd.to_datetime(time)).total_seconds())
        ) / 60 / 60
    ...
    if len(missing_times) > 0:
        missing_data = answers_data.loc[(answers_data["beiwe_id"] == user) &
                                        (answers_data["survey id"] == survey_id) &
                                        (answers_data["Local time"].isin(missing_times)), :].copy()
        
        # Get the max survey flag from agg_data so we don't overlap any survey flags after merging
        ...
        missing_data["surv_inst_flg"] = missing_data["surv_inst_flg"] + max_surv_inst_flg + 1
        
        missing_data["time_prev"] = missing_data["Local time"].shift(1)
        # one line in the survey will have a submission flag
        missing_data.loc[missing_data["time_prev"] != missing_data["Local time"], "submit_flg"] = 1
...
def read_user_answers_stream(
    ...
    Reads survey_answers data and creates a column with the survey ID, as well as a column for the
    date from the filename.
    
    Args:
        download_folder (str):
            path to downloaded data. A folder wiith the user ID should be a subdirectory of this
            path.
        user (str):
            ID of user to aggregate data
        tz_str (str):
            Time Zone to include in Local time column of output. See
            https://en.wikipedia.org/wiki/List_of_tz_database_time_zones for options
        time_start(str):
            The first date of the survey data, in YYYY-MM-DD format
        time_end(str):
            The last date of the survey data, in YYYY-MM-DD format
    ...
    Returns:
        DataFrame with stacked data, a field for the beiwe ID, a field for the survey, and a filed
    ...
    if isdirectory(ans_dir):
        ...
        for survey in survey_ids:
            ...
            if len(all_files) == 0:
                logger.info("No survey_answers for user %s in given time frames.", user)
            ...
            survey_data["question index"] = \
                survey_data.groupby(["survey id", "beiwe_id"])["question index"].cumsum()
...
def read_aggregate_answers_stream(
    ...
    Args:
        download_folder:
            path to downloaded data. This folder should have Beiwe IDs as subdirectories.
        users:
        ...
        tz_str:
            ...
            https://en.wikipedia.org/wiki/List_of_tz_database_time_zones for options
        config_path:
            Path to config file. If this is included, the function uses the config file to resolve
            semicolons that appear in survey answers lists. If this is not included, the function
            attempt to use iPhone responses to resolve semicolons.
        time_start:
            The first date of the survey data, in YYYY-MM-DD format
        time_end:
            The last date of the survey data, in YYYY-MM-DD format
        history_path:
            Path to survey history file. If this is included, the function uses the survey history
            file to find instances of commas or semicolons in answer choices
    Returns:
        DataFrame with stacked data, a field for the beiwe ID, a field for the day of week.
    """
    if time_end is None:
    ...
    all_users_list = [
        read_user_answers_stream(download_folder, user, tz_str, time_start, time_end)
        for user in users
    ]
    ...
    android_radio_rows = (aggregated_data["question type"] == "Radio Button Question")
    # Next, any rows before this date will be strings, and any after will be integers. So, we will
    # only use rows that were created after the app change happened.
    rows_after_nullable_change = (aggregated_data["UTC time"] > ANDROID_NULLABLE_ANSWER_CHANGE_DATE)
    # Finally, we will look at rows that include digits. Some people may have downloaded the app
    # before the nullable date and continued to have string answers after the date because they
    # didn't update the app, so we will filter out any answers that had strings.
    rows_with_integer_answers = (aggregated_data["answer"].apply(lambda x: str(x).isdigit()))
    ...
    for i in android_radio_questions:
        # for Androids, the radio buttons are the index of the "question answer options" list that
        # the person selected. We will iterate through and get the string corresponding to that idx
        aggregated_data.loc[i, "answer"] = \
            aggregated_data.loc[i, "question answer options"][int(aggregated_data.loc[i, "answer"])]
    ...
    # Excluding rows w/o answers to be consistent with exclusions in aggregating survey_timings
    if config_path is None:
        return aggregated_data.loc[aggregated_data["answer"] != "NOT_PRESENTED", :]
    ...
    return df_merged.loc[df_merged["answer"] != "NOT_PRESENTED", :]
...
def fix_radio_answer_choices(
    ...
    Change the "question answer options" column into a list of question answer
        options. Also, correct for the fact that a semicolon may be a delimiter between choices, an
        actual semicolon in a question, or a sanitized ","

    Args:
        aggregated_data:
            Output from read_user_answers_stream or aggregate_surveys
        config_path:
            Path to config file. If this is included, the function uses the config file to resolve
            semicolons that appear in survey answers lists. If this is not included, the function
            attempt to use iPhone responses to resolve semicolons.
        history_path:
            ...
            file to find instances of commas or semicolons in answer choices
    """
    # if a semicolon appears in an answer choice option, our regexp sub/split operation would think
    # there are way more answers than there really are. We will pull from the responses from iPhone
    # users and switch semicolons within an answer to commas. Android users have "; " separating
    # answer choices. iPhone users have ";" separating choices. This will make them the same.
    aggregated_data["question answer options"] = aggregated_data["question answer options"].apply(
    ...
    for answer_choices in radio_answer_choices_list:
        ...
        if config_path is not None:
            ...
            answer_list = aggregated_data.loc[
                aggregated_data["question answer options"] == answer_choices, "answer"
        ...
        for answer in answer_list:
            if isinstance(answer, str) and answer.find(";") != -1:
                ...
                # replace semicolons with commas within the answer. We include a space after because
                # we removed spaces after semicolons earlier.
                fixed_answer_choices = fixed_answer_choices.replace(answer, fixed_answer)
                # the answer within the answer choice string no longer has a semicolon.
        ...
        if question["question_type"] == "radio_button":
            ...
            for choice in answer_choices:
                ...
                if len(re.findall(",|;", answer_text)) != 0:
                    # At least one separation value occurs in the response
                    q_sep_choices.add(answer_text.replace(",", ";").replace("; ", ";"))
...
def get_choices_with_sep_values(
    ...
    Question IDs are included in the dict if they satisfy two conditions: 1. They are radio button
    questions 2. at least one of the response choices contains a semicolon or a comma
