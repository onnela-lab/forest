...
def get_audio_survey_id_dict(history_path: str | None = None) -> dict[str, str]:
    """ Create a dict that has most recent prompt corresponding to an audio survey as keys and the
    survey ID as the corresponding value.

    Args:
        history_path: Path to survey history file, downloaded from the Beiwe website.
...
def get_config_id_dict(config_path: str | None = None) -> dict[str, int]:
    ...
    Returns:
        dict with a key for each question prompt, and the config ID (the order of the question in
        the config file) as values
...
def read_user_audio_recordings_stream(
    ...
    Args:
        download_folder:
            path to downloaded data. A folder wiith the user ID should be a subfolder of this path.
        user:
    ...
    audio_dir = pathjoin(download_folder, user, "audio_recordings")
    ...
    for survey in survey_ids:
        ...
        # We want to be able to process the surveys even if we don't have the survey history file,
        # but we do, include the survey history file, we want to have the prompt for readability
        ...
        for i, file in enumerate(all_files):
            ...
            # Later on, we will delete all rows with blank responses. So, we want two rows with the
            # timings and an additional row to be deleted later.
        ...
        # our survey data
        data = pd.concat(survey_dfs, axis=0, ignore_index=True)
        data["beiwe_id"] = user
        data["Local time"] = \
            data["UTC time"].dt.tz_localize("UTC").dt.tz_convert(tz_str).dt.tz_localize(None)
        
        # Add question index column to make things look like the survey timings stream. We do not
        # need to worry about verifying that the question IDs on the new lines are different as we
        # did for survey timings because these are all final submissions.
        data["question index"] = 1
        data["question index"] = data.groupby(["survey id", "beiwe_id"])["question index"].cumsum()
