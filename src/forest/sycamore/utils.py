...
In the future, it would be good to refactor code and put more things in here, but for now, I'll just
include the three necessary functions so this pull request isn't massive
"""
...
def filename_to_timestamp(filename: str, tz_str: str = "UTC") -> pd.Timestamp:
    ...
    Args:
        filename(str):
            a string, with a csv file at the end formatted like "YYYY-MM-DD HH_MM_SS+00_00"
        tz_str(str):
            Output Timezone
    ...
    time_written = pd.to_datetime(str_to_convert).tz_convert(tz_str).tz_localize(None)
    return time_written
