...
def update_existing_place(
    loc_x: list, loc_y: list, num_xy: list, t_xy: list, data: np.ndarray, index: int
):
    """Update the existing significant place with the new data point.

    Args:
        loc_x: list
            a list of latitudes of significant places
        loc_y: list
...
def add_new_place(loc_x: list, loc_y: list, num_xy: list, t_xy: list, data: np.ndarray):
...
def num_sig_places(data: np.ndarray, dist_threshold: float) -> tuple[list, list, list, list]:
    ...
    for i in range(data.shape[0]):
        ...
        if distances[min_distance_index] > dist_threshold:
            ...
            update_existing_place(loc_x, loc_y, num_xy, t_xy, data[i], min_distance_index)
...
def locate_home(mob_mat: np.ndarray, timezone: str) -> tuple[float, float]:
    ...
    Args:
        mob_mat: np.ndarray
            a k*7 2d array, output from infer_mobmat()
    ...
    hours: list = []  # Initialize list to hold the hours of each observation
    
    # Convert timestamp to datetime and store the hour of each observation
    for i in range(obs_traj.shape[0]):
        time_list = stamp2datetime((obs_traj[i, 3] + obs_traj[i, 6]) / 2, timezone)
        hours.append(time_list[3])
    
    hours_arr = np.array(hours)  # Convert list to numpy array
    
    # If there are no observations between 19:00 and 09:00, raise an error
    if ((hours_arr >= 19) + (hours_arr <= 9)).sum() <= 0:
        raise RuntimeError("No home location found: Too few observations at night")
    
    # Extract the pauses (observed trajectories) that occurred between 19:00 and 09:00
    home_time_pauses = obs_traj[((hours_arr >= 19) + (hours_arr <= 9)) * obs_traj[:, 0] == 2, :]
    
    # Identify significant places during home-time pauses
    locations_lat, locations_lon, frequency, _ = num_sig_places(home_time_pauses, 20)
    
    # Find the index of the most frequently visited location, which is presumed to be home
...
def calculate_k1(
    ...
    Returns:
        similarity_measures: np.ndarray,
            A 1d array of similarity measures between the given point and each base vector.
...
def indicate_flight(
    ...
    Args:
        method: str, the method to be used for calculation, should be either 'TL', 'GL', or 'GLC'.
        current_t: float
            the current time point.
        current_x: float
            the current longitudinal position.
        current_y: float
            the current latitudinal position.
        dest_t: float
            the time point at the destination.
        dest_x: float
            the longitudinal position of the destination.
        dest_y: float
...
def adjust_direction(
    ...
    """
    This function adjusts the trajectory direction based on a given linearity parameter.
    
    Args:
        linearity: float,
            Controls the smoothness of a trajectory. A larger value tends to result in a more linear
            trajectory from the starting point towards the destination, while a smaller value leads
            to more random directions.
    ...
    origin_to_dest_norm = np.sqrt((dest_x - origin_x)**2 + (dest_y - origin_y)**2)
    
    # Generate a random number with uniform distribution within the range [0, linearity]
    smoothness_factor = np.random.uniform(low=0, high=linearity)
    
    # Adjust the displacement based on the smoothness factor
    adjusted_delta_x = delta_x + smoothness_factor * (dest_x - origin_x) / origin_to_dest_norm
    adjusted_delta_y = delta_y + smoothness_factor * (dest_y - origin_y) / origin_to_dest_norm
    
    # Calculate the norms of the initial and adjusted displacements
    initial_displacement_norm = np.sqrt(delta_x**2 + delta_y**2)
    adjusted_displacement_norm = np.sqrt(adjusted_delta_x**2 + adjusted_delta_y**2)
    
    # Normalize the adjusted displacement to maintain the same magnitude as the initial displacement
    normalized_delta_x = (adjusted_delta_x * initial_displacement_norm / adjusted_displacement_norm)
    normalized_delta_y = (adjusted_delta_y * initial_displacement_norm / adjusted_displacement_norm)
    ...
    # If the inner product is less than zero, reverse the direction of the adjusted displacement
    if inner_product < 0:
...
def multiplier(t_diff: float) -> float:
    """This function generates a multiplier based on the time difference between two points.
...
def checkbound(
    ...
    """ This function checks whether a point is out of the boundary determined by the starting
    and ending points.
    ...
    Returns:
        binary indicator, int, indicates whether (current_x, current_y) is out of the boundary
        determiend by starting and ending points
    """
    # Calculate the maximum and minimum x and y coordinates of the starting and ending points to
    # determine the boundary box of the trajectory
    max_x = max(start_x, end_x)
    min_x = min(start_x, end_x)
    max_y = max(start_y, end_y)
    min_y = min(start_y, end_y)
    # If the current point is inside of the boundary box, return 1, otherwise return 0
    if max_x + 0.01 > current_x > min_x - 0.01 and max_y + 0.01 > current_y > min_y - 0.01:
        return 1
    return 0
...
def create_tables(
    ...
    """ This function creates three tables:
        one for observed flights, one for observed pauses, and one for missing intervals.
    
    Args:
        mob_mat: 2d np.ndarray
            output from infer_mobmat()
        bv_subset: np.ndarray
            output from BV_select()
    
    Returns:
        3 2d np.ndarray
            one for observed flights, one for observed pauses, one for missing interval (where the
            last two cols are the status of previous obs traj and next obs traj)
    """
    # Number of rows in the mobility matrix and bv_subset
    mob_mat_rows = np.shape(mob_mat)[0]
    bv_set_rows = np.shape(bv_subset)[0]
    
    # Create flight table: select rows from bv_subset where first column equals 1
    flight_table = bv_subset[[bv_subset[i, 0] == 1 for i in range(bv_set_rows)], :]
    
    # Create pause table: select rows from bv_subset where first column equals 2
    pause_table = bv_subset[[bv_subset[i, 0] == 2 for i in range(bv_set_rows)], :]
    
    mis_table = np.empty((0, 8))  # Initialize missing intervals table
    
    # Iterate over mobility matrix rows to find missing intervals
    for i in range(mob_mat_rows - 1):
        # Check if end of current interval doesn't match the start of the next interval
        if mob_mat[i + 1, 3] != mob_mat[i, 6]:
            # Record missing interval along with its status (flight/pause) before and after
            mov = np.array(
                [
                    mob_mat[i, 4],
                    mob_mat[i, 5],
            ...
            mis_table = np.vstack((mis_table, mov))  # Append the missing interval to the table
...
def calculate_delta(
    ...
    """ This function calculates the displacement and time difference between two points.
    
    Args:
        flight_table: 2d np.ndarray
            output from create_tables()
        flight_index: int
            index of the flight in flight_table
        backwards: bool
            whether the flight is backwards
    
    Returns:
        delta_x, delta_y, delta_t: float,
            displacement and time difference between two points
    """
...
def adjust_delta_if_needed(
    start_t: float, delta_t: float, delta_x: float, delta_y: float, end_t: float
) -> tuple[float, float, float]:
    """ This function adjusts the displacement and time difference between two points
    ...
    Returns:
        delta_x, delta_y, delta_t: float,
            adjusted displacement and time difference between two points
    """
    if start_t + delta_t > end_t:
        temp = delta_t
...
def calculate_position(
    ...
    return time_part2 / time_diff * start_delta + time_part1 / time_diff * end_delta


def update_table(imp_table: np.ndarray, list_elem: list) -> np.ndarray:
...
def forward_impute(
    ...
    linearity: float,
    mis_row: np.ndarray,
    pause_table: np.ndarray,
    imp_table: np.ndarray,
    start_s: int,
    method: str,
    counter: int,
) -> tuple[np.ndarray, tuple[int, float, float, float], int]:
    """ This function imputes a missing interval from the start point to the end point.
    ...
    Args:
        ...
        mis_row: np.ndarray
            a row of missing interval table
        pause_table: np.ndarray
            output from create_tables()
        imp_table: np.ndarray
            output from create_tables()
        start_s: int
            status of the start point
        method: str
            the method to be used for calculation, should be 'TL', 'GL', or 'GLC'
        counter: int
            number of imputed trajectories
    
    Returns:
        imp_table: updated imp_table tuple of 4 elements:
            start_s: int, status of the start point start_t: float
                start time
            start_x, start_y: float
                start position
    ...
    condition = (
        (sum(flight_indication == 1) == switch and start_s == 2) or
    ...
    if condition:
        ...
        try_y = calculate_position(start_t, end_t, try_t, start_y + delta_y, end_y)
        
        mov1 = great_circle_dist(try_x, try_y, start_x, start_y)[0]
        mov2 = great_circle_dist(end_x, end_y, start_x, start_y)[0]
        
        check1 = checkbound(try_x, try_y, *mis_row[[0, 1, 3, 4]])
        check2 = int(mov1 < mov2)
        
        # conditions and actions
        if end_t > start_t and check1 == 1 and check2 == 1:
            current_t = start_t + delta_t
            current_x = calculate_position(start_t, end_t, current_t, start_x + delta_x, end_x)
            current_y = calculate_position(start_t, end_t, current_t, start_y + delta_y, end_y)
            imp_table = update_table(
                imp_table, [1, start_x, start_y, start_t, current_x, current_y, current_t]
            )
        ...
        if end_t > start_t and check2 == 0:
            ...
            imp_table = update_table(
                imp_table, [1, start_x, start_y, start_t, end_x, end_y, current_t]
            )
            ...
            if start_t + delta_t < end_t:
                current_t = start_t + delta_t
                imp_table = update_table(
                    imp_table, [2, start_x, start_y, start_t, start_x, start_y, current_t]
                )
                start_t, start_s = current_t, 2
                counter += 1
            else:
                imp_table = update_table(
                    imp_table, [1, start_x, start_y, start_t, end_x, end_y, end_t]
                )
                start_t = end_t
...
def backward_impute(
    ...
    Args:
        ...
        method: str
            the method to be used for calculation, should be either 'TL', 'GL', or 'GLC'
        counter: int
            number of imputed trajectories
    ...
    if condition:
        ...
        delta_x, delta_y, delta_t = adjust_delta_if_needed(
            start_t, delta_t, delta_x, delta_y, end_t
        )
        ...
        try_t = end_t - delta_t
        try_x = calculate_position(start_t, end_t, try_t, start_x, end_x + delta_x)
        try_y = calculate_position(start_t, end_t, try_t, start_y, end_y + delta_y)
        ...
        if end_t > start_t and check1 == 1 and check2 == 1:
            ...
            current_x = calculate_position(start_t, end_t, current_t, start_x, end_x + delta_x)
            current_y = calculate_position(start_t, end_t, current_t, start_y, end_y + delta_y)
            imp_table = update_table(
                imp_table, [1, current_x, current_y, current_t, end_x, end_y, end_t]
            )
        ...
        if end_t > start_t and check2 == 0:
            ...
            imp_table = update_table(
                imp_table, [1, start_x, start_y, current_t, end_x, end_y, end_t]
            )
            ...
            if start_t + delta_t < end_t:
                current_t = end_t - delta_t
                imp_table = update_table(
                    imp_table, [2, end_x, end_y, current_t, end_x, end_y, end_t]
                )
...
def impute_gps(
    ...
    # create three tables for observed flights, observed pauses, and missing intervals
    ...
    imp_table = np.zeros((1, 7))  # initialize the imputed trajectory table
    ...
    for i in range(mis_table.shape[0]):
        ...
        # get the distance difference between start and end coordinates of the missing interval
        distance_difference = great_circle_dist(*mis_table[i, [0, 1]], *mis_table[i, [3, 4]])[0]
        
        # get the time difference between start and end times of the missing interval
        time_difference = mis_t1 - mis_t0
        
        # get the distance between the start location of the missing interval and the home location
        start_home_distance = great_circle_dist(mis_table[i, 0], mis_table[i, 1], *home_coords)[0]
        # get the distance between the end location of the missing interval and the home location
        end_home_distance = great_circle_dist(mis_table[i, 3], mis_table[i, 4], *home_coords)[0]
        ...
        # if the distance difference is more than 300 km, we assume the person takes a flight
            ...
            if speed > 210:
                ...
                # if the speed is less than 210 m/s, generate a random speed between 244 and 258 m/s
                ...
                # generate a random start time from the start time of the missing interval to the
                # end time of the missing interval minus the time needed to travel the distance
        ...
        # add one more check about how many flights observed in the nearby 24 hours
            ...
            # if distance difference is less than 3 km, generate a random speed between 1 and 1.8m/s
            if distance_difference < 3000:
                random_speed = np.random.uniform(low=1, high=1.8)
            # if distance difference is more than 3 km, generate a random speed between 13 and 32m/s
            else:
                random_speed = np.random.uniform(low=13, high=32)
            
            t_need = min(distance_difference / random_speed, time_difference)
            # if the time needed to travel the distance is less than the time difference,
            ...
            # solve for the problem where a person has a trajectory like
            # flight/pause/flight/pause/flight...
            ...
            start_x, start_y, start_t, end_x, end_y, end_t, start_s, end_s = mis_table[i, :]
            if time_difference > 4 * 60 * 60 and min(start_home_distance, end_home_distance) <= 50:
                ...
                # if distance from home to start/end is less than 50 km and the time difference is
                # more than 4 hours set the start/end time to be the same and the start/end location
                # to be the same
                if start_home_distance <= 50:
                    imp_table = update_table(
                        imp_table, [2, start_x, start_y, start_t, start_x, start_y, end_t - t_need]
                    )
                    start_t = end_t - t_need
                else:
                    imp_table = update_table(
                        imp_table, [2, end_x, end_y, start_t + t_need, end_x, end_y, end_t]
                    )
            ...
            while start_t < end_t:
                # if start and end location are different and the time difference is less than 30s
                
                if abs(start_x - end_x) + abs(start_y - end_y) > 0 and end_t - start_t < 30:
                ...
                # if start and end location are the same
                    ...
                    imp_table = update_table(
                        imp_table, [2, start_x, start_y, start_t, end_x, end_y, end_t]
                    )
                    start_t = end_t
                
                # if start and end location are different and the time difference is more than 30s
                else:
...
def imp_to_traj(
    ...
    """ This function tidies up the first-step imputed trajectory, such as combining pauses, flights
    shared by both observed and missing intervals, also combine consecutive flight with slightly
    different directions as one longer flight

    Args:
        imp_table: 2d array
            the first-step imputed trajectory
        mob_mat: 2d array
            output from infer_mobmat()
        w: float
            the weight for the distance
    
    Returns:
        2d array, the final imputed trajectory, with one more columm compared to imp_table which is
        an indicator showing if the piece of traj is imputed (0) or observed (1)
    """
    logger.info("Tidying up the trajectories...")
    
    mis_table = np.empty((0, 8))  # Create a table for missing values
    ...
    for k in range(mis_table.shape[0]):
        ...
        while start_idx < temp.shape[0]:
            ...
            if (
                end_idx == temp.shape[0] or
                temp[min(end_idx, temp.shape[0] - 1), 0] != temp[start_idx, 0]
            ):
                start = start_idx
                end = end_idx - 1
                start_idx = end_idx
                ...
                # added some extra whitespace below to assist in readability (commas)
                
                # If the trajectory is multiple pauses, add it to the tidy_trajectory
                if temp[start, 0] == 2:
                    tidy_trajectory.append([2,  *temp[start, 1:4],  *temp[end, 4:7]])
                
                # If the trajectory is a flight in one row, add it to the tidy_trajectory
                elif end == start:
                    tidy_trajectory.append([1,  *temp[start, 1:4],  *temp[end, 4:7]])
                else:
                    # More complex case when multiple pauses/trajectories are combined
                    ...
                    while complete == 0:
                        ...
                        for i, mat_elem in enumerate(mat_list):
                            knot_yes[i], knot_pos[i] = exist_knot(mat_elem, w)
                        ...
                        if sum(knot_yes) == 0:
                            ...
                            for i, mat_elem in enumerate(mat_list):
                                if knot_yes[i] == 1:
                                    knots.append( int( (mat_elem)[ int(knot_pos[i]) , 3] ))
                            knots.sort()
                    
                    for j in range(len(knots) - 1):
                        tidy_trajectory.append([1,  *mat[knots[j], 0:3],  *mat[knots[j + 1], 0:3]])
