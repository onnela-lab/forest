...
Forest is a library for analyzing smartphone-based high-throughput digital phenotyping data collected with the [Beiwe platform](http://www.beiwe.org/). Forest implements methods as a Python package. Forest is integrated into the Beiwe back-end on AWS but can also be run locally.
...
Input data should be included in one folder, usually sourced from one Beiwe Platform Study. Inside that folder there should be a direct subfolder corresponding to each participant ID. Inside each Beiwe ID subfolder, there should be a direct subfolder corresponding to each downloaded data stream.

Methods are designed to work on data collected using the Beiwe app, with the types of sensor on/off cycles run by the Beiwe app, and csv files containing columns generated by Beiwe, or generated data matching the data returned from the Beiwe app. Some methods included in Forest are compatible with other data collection environments, but code changes would be required to make Forest work well with those environments. Forest can also consume these csv files compressed using the `zstd` data compression standard in `.zst` files.

You can automate and speed up downloading your data using the Mano Python package. You can find instructions to install it here: [https://github.com/harvard-nrg/mano](https://github.com/harvard-nrg/mano)
...
Output files are named from their source participant id, contained in a folder indicating the specified _frequency_ component, such as `daily` or `hourly`.
...
The following columns are created in the "submits_and_deliveries.csv" file. This file will only be generated if the config file and intervention timings file are provided.
...
The following columns are created in the "answers_data.csv" file. This file will be generated if a survey config file is available.
```{list-table}
:header-rows: 1
* - Variable
  - Type
...
The following columns are created in the "answers_summary.csv" file. This file will only be generated if the config file and intervention timings file are provided.
```{list-table}
:header-rows: 1
* - Variable
  - Type
...
The following columns are created in the "submits_only.csv" file. This file will always be generated.
```{list-table}
:header-rows: 1
* - Variable
  - Type
...
The following columns are created in a csv file for each survey.
```{list-table}
:header-rows: 1
* - Variable
  - Type
...
[Beiwe Documentation](https://github.com/onnela-lab/beiwe-backend/wiki)
