...
def convert_time_to_date(submit_time, day, time):
    ...
    days = [submit_time + 
            datetime.timedelta(days=i) for i in range(0 - dow, 7 - dow)]
    ...
    days = [[d.replace(hour=t[0], minute=t[1], second=t[2], microsecond=0) 
             for t in time] for d in days]

    return days[day]


def generate_survey_times(time_start, 
                          time_end, 
                          timings=[], 
                          survey_type='weekly'):
    """
    ...
    start_dates = [time_start + datetime.timedelta(days=7 * (i)) 
...
def survey_submits(study_dir, config_path, time_start, time_end, beiwe_ids, 
                   agg, study_tz=None):
    """
    Args:
        study_dir(str):
    ...
    submit_lines = pd.merge(
        sched[['delivery_time', 
               'next_delivery_time', 'id', 'beiwe_id']].drop_duplicates(),
        agg[['Local time', 'config_id', 'survey id', 
             'beiwe_id']].loc[agg.submit_line == 1].drop_duplicates(),
        how='left',
        left_on=['id', 'beiwe_id'],
    ...
    submit_lines2 = submit_lines.groupby(['delivery_time', 
        'next_delivery_time', 'survey id', 'beiwe_id', 'config_id'])[
    ...
    submit_lines3['submit_time'] = np.where(submit_lines3.submit_flg == 1, 
    ...
    # Create a summary that has survey_id, beiwe_id, num_surveys, 
    # num submitted surveys, average time to submit
    summary_cols = ['survey id', 'beiwe_id']
    num_surveys = submit_lines3.groupby(summary_cols)['submit_flg'].count()
    num_complete_surveys = submit_lines3.\
        groupby(summary_cols)['submit_flg'].sum()
    avg_time_to_submit = submit_lines3.loc[submit_lines3.submit_flg == 1].\
        groupby(summary_cols)['time_to_submit'].apply(
        lambda x: sum(x, datetime.timedelta()) / len(x))

    submit_lines_summary = pd.concat([num_surveys, num_complete_surveys, 
                                      avg_time_to_submit], 
                                     axis=1).reset_index()
    submit_lines_summary.columns = ['survey id', 'beiwe_id', 'num_surveys', 
...
def survey_submits_no_config(study_dir, study_tz=None):
    ...
    tmp = tmp.groupby(['survey id', 'beiwe_id', 
                       'surv_inst_flg'])['Local time'].\
