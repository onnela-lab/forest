...
### Write a function that subsets the list if it starts with nan and ends with 
...
def agg_changed_answers(study_dir, config_path, agg, study_tz=None):
    ...
    Returns:
        agg(DataFrame):
            ...
            with changed answers aggregated into a list. The Final answer is 
            in the 'last_answer' field
    """
    ...
    # add in an answer ID and take the last of that too to join back 
    # on the time
    ...
    agg['all_answers'] = agg['all_answers'].apply(lambda x: 
        x if isinstance(x, float) else subset_answer_choices(x))
    agg['num_answers'] = agg['all_answers'].apply(lambda x: 
        x if isinstance(x, float) else len(list(x)))
    agg = agg.reset_index()
...
def agg_changed_answers_summary(study_dir, config_path, agg, study_tz=None):
    ...
    surv_config['q_internal_id'] = surv_config.groupby('config_id').\
        cumcount() + 1

    detail = pd.merge(detail, surv_config[['question_id', 'q_internal_id']], 
        how='left', left_on='question id', right_on='question_id')

    detail['instance_id'] = np.where(detail['q_internal_id'] == 1, 1, 0)
    detail['instance_id'] = detail['instance_id'].cumsum()

    detail['last_time_1'] = detail.groupby(['instance_id'])['last_time'].\
        shift(1)
    ...
    summary_cols = ['survey id', 'beiwe_id', 'question id', 
                    'question text', 'question type']
    num_answers = detail.groupby(summary_cols)['num_answers'].count()
    avg_time = detail.groupby(summary_cols)['time_to_answer'].apply(lambda x: 
            sum(x, datetime.timedelta()) / len(x))
    avg_chgs = detail.groupby(summary_cols)['num_answers'].mean()
    most_common_answer = detail.groupby(summary_cols)['all_answers'].apply(
        lambda x: max(set([x for x in x for x in x]), 
                      key=[x for x in x for x in x].count, default=0))

    out = pd.concat([num_answers, avg_time, avg_chgs, most_common_answer], 
                    axis=1).reset_index()

    out.columns = summary_cols + ['num_answers', 'average_time_to_answer', 
        'average_number_of_answers', 'most_common_answer']
