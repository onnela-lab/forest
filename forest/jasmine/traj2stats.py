...
def gps_summaries(traj,tz_str,option):
    ...
    try:
        ...
        if option == "daily":
            ...
            h = np.round((end_stamp - start_stamp)/60/60/24,0)
            window = 60*60*24
        ...
        if h>=1:
            for i in range(int(h)):
                ...
                if option=="hourly":
                    if obs_dur==0:
                        summary_stats.append([year,month,day,hour,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan])
                    else:
                        summary_stats.append([year,month,day,hour,obs_dur/60,total_pause_time/60,total_flight_time/60,time_at_home/60,
                ...
                if option=="daily":
                    ...
                    if temp.shape[0]==1:
                        ...
                        diameter = max(D)
                    if obs_dur == 0:
                        summary_stats.append([year,month,day,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan])
                    else:
                        summary_stats.append([year,month,day,obs_dur/3600, obs_day/3600, obs_night/3600,total_pause_time/3600,total_flight_time/3600,
                                  time_at_home/3600, max_dist_home/1000, dist_traveled/1000,av_f_len/1000,sd_f_len/1000,av_f_dur/3600,sd_f_dur/3600,
                                  radius/1000, diameter/1000, num_sig, entropy])
            summary_stats = pd.DataFrame(np.array(summary_stats))
            if option == "hourly":
                summary_stats.columns = ["year","month","day","hour","obs_duration","pause_time","flight_time","home_time","max_dist_home",
                                "dist_traveled","av_flight_length","sd_flight_length","av_flight_duration","sd_flight_duration"]
            if option == "daily":
                summary_stats.columns = ["year","month","day","obs_duration","obs_day","obs_night","pause_time","flight_time","home_time",
                                       "max_dist_home","dist_traveled","av_flight_length","sd_flight_length","av_flight_duration",
                                       "sd_flight_duration","radius","diameter","num_sig_places","entropy"]
        else:
            if option == "hourly":
                summary_stats = pd.DataFrame(columns=["year","month","day","hour","obs_duration","pause_time","flight_time","home_time",
                                "max_dist_home","dist_traveled","av_flight_length","sd_flight_length","av_flight_duration","sd_flight_duration"])
            if option == "daily":
                summary_stats = pd.DataFrame(columns=["year","month","day","obs_duration","obs_day","obs_night","pause_time","flight_time",
                                "home_time","max_dist_home", "dist_traveled","av_flight_length","sd_flight_length","av_flight_duration",
                                "sd_flight_duration","radius","diameter","num_sig_places","entropy"])
    ...
    for ID in beiwe_id:
        try:
            ...
            if quality>0.6:
                ...
                sys.stdout.write("GPS data quality is too low to impute for user %s." % str(ID) + '\n')
                logger.warning("GPS data quality is too low to impute for user %s." % str(ID))
        except:
