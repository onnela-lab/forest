"""
This tree is dedicated to quantification of various aspects of physical activity using 
accelerometer data.
...
def adjust_bout(vm_bout,fs):
    ...
    E.g., if the bout is 9.8s long, function fills in values at its end to 
    make it 10s (results in N%fs=0).
...
def find_walking(vm_bout,fs,I,step_freq,alpha,beta,epsilon,delta):
    """
    ...
    # calculate pp to exclude periods of low-intensity activity from further computing
    x = vm_bout.reshape(fs,-1,order="F")
    pp = np.array([max(x[:,i])-min(x[:,i]) for i in range(x.shape[1])])
    valid = np.ones(len(pp),dtype=bool)
    valid[pp<I] = False
    
    if sum(valid)>=epsilon: # compute cwt only if valid fragment of bout is sufficiently long
        ...
        tapered_bout = np.concatenate((np.zeros(5*fs),(tapered_bout)*w,np.zeros(5*fs)))
        ...
        for i in range(int(coefs_interp.shape[1]/fs)):
            ...
            window = np.sum(coefs_interp[:,np.arange(xStart,xFinish)],axis=1)
...
def find_continuous_dominant_peaks(E,epsilon,delta):
    ...
    for m in range(E.shape[1]-epsilon):
        ...
        loop = [i for i in np.arange(epsilon)] + [i for i in np.arange(epsilon-2,-1,-1)]
        for t in range(len(loop)):
            ...
            if j == 0:
                ...
                break
        B[:,np.arange(m,m+epsilon)] = np.maximum(B[:,np.arange(m,m+epsilon)],A)

    B=B[:,:-1]
    return B

def main_function(study_folder:str, output_folder:str, tz_str:str, option:str, time_start=None, time_end=None,beiwe_id=None):
    """
    ...
    if option == None or option == 'both' or option == 'daily':
        if os.path.exists(output_folder+"/daily")==False:
            os.mkdir(output_folder+"/daily")
    if option == None or option == 'both' or option == 'hourly':
    ...
    for ID in beiwe_id:
        ...
        days_hourly = pd.date_range(date_start,date_end+timedelta(days=1),freq='H')
        ...
        for d, d_datetime in enumerate(days):
            ...
            for f in file_ind:
                ...
                try:
                    ...
                    samples_enough = samples_per_sec>=(fs-1) # seconds with enough samples / 9 should be in fact fs
                    
                    # find bouts with sufficient duration (here, minimum 5s)
                    N,BI,B = rle(samples_enough) # N=Running length, BI=starting index, B=value
                    bout_start = BI[(B==True) & (N>=5)]
                    bout_duration = N[(B==True) & (N>=5)]

                    for b, b_datetime in enumerate(bout_start):
                        # create a list with second-level timestamps present in this bout
                        bout_time = pd.date_range(t_sec_bins[bout_start[b]],t_sec_bins[bout_start[b]+bout_duration[b]],freq='S').tolist()
                        ...
                        if np.sum([np.std(x_bout),np.std(y_bout),np.std(z_bout)])>minimum_activity_thr:
                            # interpolate bout to 10Hz and calculate vm
                            [x_bout,y_bout,z_bout,vm_bout]=standardize_bout(t_bout,x_bout,y_bout,z_bout)
                            ...
                            ## activity types
                            # gait
                            cadence_bout = find_walking(vm_bout,fs,I,step_freq,alpha,beta,epsilon,delta)
                                
                            cadence_bout = cadence_bout[np.where(cadence_bout>0)]
                            if option == None or option == 'both' or option == 'daily': 
                                cadence_temp_daily = np.concatenate((cadence_temp_daily,cadence_bout))
                            if option == None or option == 'both' or option == 'hourly':
                                cadence_temp_hourly = np.concatenate((cadence_temp_hourly,cadence_bout))
        ...
        if option == None or option == 'both' or option == 'hourly':
           summary_stats = pd.DataFrame({'date': days_hourly[:-1].strftime('%Y-%m-%d %H:%M:%S'),
                                         'walking_time': walkingtime_hourly.flatten(),
...
I = 0.3 # minimum peak-to-peak amplitude (in gravitational units (g))
step_freq = [1.4,2.3] # step frequency (in Hz)
alpha = 0.6 # maximum ratio between dominant peak below and within step frequency range
beta = 2.5 # maximum ratio between dominant peak above and within step frequency range
delta = 20 # maximum change of step frequency between two one-second nonoverlapping segments (multiplication of 0.05Hz, e.g., delta=2 -> 0.1Hz)
...
study_folder = "C:/Users/mstra/Documents/data/beiwe_test_data/onnela_lab_ios_test2"
...
# main function
main_function(study_folder, output_folder, tz_str, option=None, time_start=None, time_end=None,beiwe_id={"sxvpopdz"})